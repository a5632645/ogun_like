...
namespace mana {
CurveV2::CurveV2(int size, CurveInitEnum init) {
    max_line_resolution_ = size;
    curr_line_resolution_ = size;
...
void CurveV2::SetCurrLineResolution(int resolution) {
    curr_line_resolution_ = resolution;
    FullRender();
}
...
void CurveV2::PartRender(int begin_point_idx, int end_point_idx) {
    ...
    for (int i = begin_point_idx; i < end_point_idx; ++i) {
        ...
        auto begin_idx = static_cast<int>(std::round(curr_point.x * curr_line_resolution_));
        auto end_idx = static_cast<int>(std::round(next_point.x * curr_line_resolution_));
    ...
    datas_[curr_line_resolution_] = datas_[curr_line_resolution_ - 1];
    datas_[curr_line_resolution_ + 1] = datas_[curr_line_resolution_];
}
